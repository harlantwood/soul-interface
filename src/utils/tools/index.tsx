...
export function getLocalConfig (
  ...
  if (curVersion && curVersion !== bridgeNetworks.version) {
  ...
  if (!lstr) {
    ...
    if (!lboj[chainID]) {
      ...
      (lboj[chainID][account].timestamp < bridgeNetworks.localDataDeadline && token !== 'all')
      || (lboj[chainID][account].timestamp < bridgeNetworks.localDataDeadline && token === 'all')
    ) { // 在某个时间之前的数据无效
...
export function setLocalConfig (
  ...
  localStorage.setItem(version + '_' + VERSION, bridgeNetworks.version)
...
//   } else {
//     return formatDecimal(num, bridgeNetworks.keepDec)
//   }
// }
