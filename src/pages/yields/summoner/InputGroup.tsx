...
import { useHistory } from 'react-router-dom/main'
import useMasterChef from 'hooks/useMasterChef'
import usePendingSoul from 'hooks/usePendingSoul'
import useStakedBalance from 'hooks/useStakedBalance'
import useTokenBalance from 'hooks/useTokenBalance'
// import { formatNumber, isAddressString, isWETH } from 'utils'
...
export default function InputGroup({
    ...
    const pairAddressChecksum = validateAndParseAddress(pairAddress)
    ...
    const [approvalState, approve] = useApproveCallback(
        ...
        MASTERCHEF_ADDRESS[chainId || 333999]
    )
            ...
            <div className="flex flex-col space-y-4 py-6">
                <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 px-4">
                    {type === 'LP' && (
                        <>
                            <Button
                                color="default"
                                onClick={() => history.push(`/add/${isNative(token0Address)}/${isNative(token1Address)}`)}
                            ...
                            <Button
                                color="default"
                                onClick={() =>
                                    history.push(`/remove/${isNative(token0Address)}/${isNative(token1Address)}`)
                ...
                <div className="grid gap-4 grid-cols-2 px-4">
                    ...
                    <div className="text-center col-span-2 md:col-span-1">
                        {account && (
                            <div className="text-sm text-secondary cursor-pointer text-right mb-2 pr-4">
                                ...
                                {formatNumber(fixedFormatting(balance.value, balance.decimals))} {type}
                            </div>
                        )}
                    ...
                    <div className="text-center col-span-2 md:col-span-1">
                        {account && (
                            <div className="text-sm text-secondary cursor-pointer text-right mb-2 pr-4">
                                {i18n._(t`Deposited`)}: {formatNumber(fixedFormatting(staked.value, staked.decimals))}{' '}
                                {type}
                            </div>
                        )}
                        <div className="flex items-center relative w-full mb-4">
                ...
                {pending && Number(pending) > 0 && (
                    <div className=" px-4">
                        ...
                        >
                            {i18n._(t`Harvest ${formatNumber(pending)} SOUL`)}
                        </Button>
                    </div>
                )}
            </div>
