import { Binance, ChainId, Currency, Ether, Token, WNATIVE } from '../../sdk' // Moonriver
...
export default function Bridge() {
  ...
  const [chainTo, setChainTo] = useState<Chain | null>(
    chainId == ChainId.FANTOM ? DEFAULT_CHAIN_FROM : DEFAULT_CHAIN_TO
  )
  ...
  const anyswapCurrencyContract = useAnyswapTokenContract(
    currency0 && currency0.chainId == ChainId.FANTOM && tokenToBridge.other.ContractAddress,
    true
  )
  ...
  const handleChainFrom = useCallback(
    (chain: Chain) => {
      ...
      if (changeTo.id !== ChainId.FANTOM && chain.id !== ChainId.FANTOM) {
  ...
  const handleChainTo = useCallback(
    (chain: Chain) => {
      ...
      if (changeFrom.id !== ChainId.FANTOM && chain.id !== ChainId.FANTOM) {
  ...
  const bridgeToken = async () => {
    ...
    const depositAddress = currency0.chainId == ChainId.FANTOM ? token.ContractAddress : token.DepositAddress

    const amountToBridge = ethers.utils.parseUnits(currencyAmount, token.Decimals)
    setPendingTx(true)

    try {
      if (currency0.chainId == ChainId.FANTOM) {
