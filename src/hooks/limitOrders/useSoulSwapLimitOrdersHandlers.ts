...
import { useTransactionAdder } from "state/limit-transactions/hooks";
import useSoulSwapLimitOrdersLib from "./useSoulSwapLimitOrdersLib";
import { useActiveWeb3React } from "services/web3";

export declare const isEthereumChain: (chainId: number) => boolean;

export interface SoulSwapLimitOrdersHandlers {
...
export default function useSoulSwapLimitOrdersHandlers(): SoulSwapLimitOrdersHandlers {
  const { chainId, account } = useActiveWeb3React();

  const gelatoLimitOrders = useSoulSwapLimitOrdersLib();
  ...
  const handleLimitOrderSubmission = useCallback(
    async (
      ...
      if (!gelatoLimitOrders) {
        throw new Error("Could not reach SoulSwap Limit Orders library");
      }
